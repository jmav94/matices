...
if ( isset($_POST['e_nombre']) ){
	...
	$query = "SELECT * FROM servicios_destinatarios WHERE idservicio='".$id."' LIMIT 1";
	if ( mysql_num_rows(mysql_query($query)) ){
		mysql_query("UPDATE servicios_destinatarios SET nombre='".$e_nombre."',direccion='".$e_direccion."',fechaentrega='".$e_fechaentrega."',horaentrega='".$e_horaentrega."',referencia='".$e_referencia."',colonia='".$e_colonia."',codigopostal='".$e_codigopostal."',mensaje='".$e_mensaje."' WHERE idservicio='".$id."'") or die(mysql_error());
	} else {
		mysql_query("INSERT INTO servicios_destinatarios SET nombre='".$e_nombre."',direccion='".$e_direccion."',fechaentrega='".$e_fechaentrega."',horaentrega='".$e_horaentrega."',referencia='".$e_referencia."',colonia='".$e_colonia."',codigopostal='".$e_codigopostal."',mensaje='".$e_mensaje."',idservicio='".$id."'") or die(mysql_error());
	}
...
if ( isset($_POST['estado']) ){
	...
	mysql_query("INSERT INTO servicios_estados SET fecha='".$fecha."',hora='".$hora."',estado='".$estado."',comentarios='".$comentarios."',idservicio='".$id."'");
	mysql_query("UPDATE servicios SET estatus='".$estado."' WHERE idservicios='".$id."'");
}

$data = mysql_fetch_object(mysql_query("SELECT * FROM servicios JOIN clientes ON clientes.idclientes=servicios.idcliente WHERE idservicios='".$id."' LIMIT 1"));
?>
...
<?php
				...
				if ( mysql_num_rows(mysql_query($query)) ){
					...
					echo '<a href="#" class="agregarDatos btn btn-md btn-default btn-block m-t" data-id="'.$dest->idservicio.'"> <i class="fa fa-pencil"></i> Editar datos </a>';
...
<?php
						...
						while($q = mysql_fetch_object($query)){
                    		...
                    		$compo = mysql_query("SELECT * FROM articulos_componentes WHERE idarticulo='".$q->idarticulos."' ORDER BY idcomponentes ASC");
                    		while($c = mysql_fetch_object($compo)){
                    			echo '<tr>
                    				<td>&nbsp;&nbsp;&nbsp;<strong>- Componente:</strong> '.$c->componente.'</td>
                    				<td class="text-center">'.($c->cantidad * $q->cantidad).'</td>
                    			</tr>';	
                    		}
...
<?php
						$query = mysql_query("SELECT * FROM servicios_estados WHERE idservicio='".$data->idservicios."' ORDER BY idestados DESC");
						while($q = mysql_fetch_object($query)){
					...
					<div class="col-sm-12">
						...
						<form role="form" action="" class="form-horizontal" method="post">
							<div class="form-group">
								...
								<div class="col-md-8"><input type="text" class="form-control" name="e_nombre" id="e_nombre"></div>
							</div>
							<div class="form-group">
								<label class="col-md-4 control-label"><strong>Dirección</strong></label>
								<div class="col-md-8"><input type="text" class="form-control" name="e_direccion" id="e_direccion"></div>
							</div>
							<div class="form-group">
								<label class="col-md-4 control-label"><strong>Fecha de Entrega</strong></label>
								<div class="col-md-8"><input type="text" class="form-control" name="e_fechaentrega" id="e_fechaentrega" value="<?php echo date("Y-m-d"); ?>" data-date-format="yyyy-mm-dd"></div>
							</div>
							<div class="form-group">
								<label class="col-md-4 control-label"><strong>Hora de Entrega</strong></label>
								<div class="col-md-8"><input type="text" class="form-control" name="e_horaentrega" id="e_horaentrega"></div>
							</div>
							<div class="form-group">
							...
							<div class="form-group">
								<label class="col-md-4 control-label"><strong>Mensaje</strong></label>
								<div class="col-md-8"><textarea style="height:100px;" class="form-control" name="e_mensaje" id="e_mensaje"></textarea></div>
							</div>
...
<script>
	$(function(){
		...
		$(".agregarDatos").click(function(){
			...
			if (id){
				$.getJSON("php/ajax/servicios_destinatario.php?id="+id, function(e){
